...
//	private HashMap<Component.Identifier, ComponentStateInfo> componentStates;
	...
	// Component:AbstractInputAction pairs into the tage.actions table, by calling
	// method
	// associateAction().
//	private HashMap<Component.Identifier, IAction> tage.actions;
	...
	public InputManager(int verbosity) {
		...
		*/
		...
		//create a hashtable entry for each controller to hold tage.actions associated with the controller's components
		for (Controller c : controllers) {
	...
	public void update(float time) {
		...
		for (Controller c : controllers) {
			...
			while (queue.getNextEvent(event)) { // get an event
				...
				// which tage.actions have been registered will have ComponentStateInfo objects
				if (componentState != null) {
					...
					if (event.getComponent().getIdentifier() instanceof Component.Identifier.Axis) {
						...
						if (prettyCloseTo(Math.abs(event.getValue()), 1.0f)) {
							...
							// check if component tage.actions should be invoked on press
							...
							// check if component tage.actions should be invoked "ON_RELEASED"
		...
		// invoke tage.actions for all button/key components which are currently pressed and
			...
			for (Component.Identifier component : controllerStates.get(c).keySet()) {
				...
				if (componentState.isNewEvent()) {
					...
					if (componentState.isPressed()
						...
						// check for repeatable axis tage.actions.
					...
					}// end else check for for repeatable axis tage.actions
